/* **********************************************************
 * OpenMP Parallel Code : Conways' game of life
 * 
...
int main(int argc,char **argv)
{
    ...
    /* Enable nested parallelism for sections with parallel regions inside */
    omp_set_nested(1);
    ...
    /* Main iteration loop with combined task and data parallelism */
    for(t=0;t<maxiter && population[w_plot];t++)
    {
        /* Reset population counter for this iteration */
        population[w_update] = 0;
        
        /* Task parallelism: Use sections to separate computation and plotting */
        #pragma omp parallel sections
        {
            /* Section 1: Computation with data parallelism */
            #pragma omp section
            {
                /* All threads in this section participate in computation */
                /* One-dimensional row-wise decomposition */
                int comp_threads = (numthreads > 1 && !disable_display) ? numthreads - 1 : numthreads;
                if (comp_threads < 1) comp_threads = 1;
                
                /* Data parallelism: divide rows among threads */
                #pragma omp parallel num_threads(comp_threads) reduction(+:population[w_update])
                {
                ...
                /* Pointer swap after computation completes */
                tmesh = nextWorld;
                nextWorld = currWorld;
                currWorld = tmesh;
            ...
            /* Section 2: Plotting (single thread) */
            #pragma omp section
            {
                if(!disable_display) {
                    /* Wait for computation and swap to complete */
                    /* Implicit barrier at end of sections ensures synchronization */
